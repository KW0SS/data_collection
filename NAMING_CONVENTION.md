...
**형식:** `data/output/{GICS섹터}/{종목코드}_{연도}.csv`

GICS 섹터별 서브디렉터리로 분류됩니다.

```
data/output/
├── Health Care/
│   ├── 052670_2010.csv     ← 제일바이오 2010년
...
| 구성 요소 | 형식 | 예시 |
|---|---|---|
| GICS 섹터 | 영문 (아래 표 참조) | `Health Care` |
| 종목코드 | 6자리 숫자 | `019440` |
| 연도 | 4자리 숫자 | `2023` |
| 구분자 | `_` (언더스코어) | |
| 확장자 | `.csv` | |

> `gics_sector`가 없는 기업은 `Unknown/` 디렉터리에 저장됩니다.
